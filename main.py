...
def run_auto_screener(config: Config, template_name: Optional[str] = None):
    """
    运行自动选股：执行所有 auto_run=True 的模板，或指定模板。
    """
    from src.services.screener_service import ScreenerService

    service = ScreenerService()
    service.ensure_builtin_templates()

    notify = getattr(config, 'screener_notify', True)

    if template_name:
        # 按名称查找模板
        templates = service.list_templates()
        matched = [t for t in templates if t.get("name") == template_name]
        if not matched:
            logger.error(f"选股模板 '{template_name}' 不存在")
            return
        for t in matched:
            logger.info(f"运行选股模板: {t['name']}")
            result = service.run_screen(template_id=t['id'], notify=notify)
            logger.info(
                f"选股完成: {t['name']} - 扫描 {result.get('total_scanned')} 只, "
                f"命中 {result.get('matched_count')} 只, 耗时 {result.get('duration_seconds', 0):.1f}s"
            )
    else:
        # 运行所有 auto_run 模板
        from src.repositories.screener_repo import ScreenerRepository
        repo = ScreenerRepository()
        auto_templates = repo.get_auto_run_templates()
        if not auto_templates:
            logger.info("没有设置 auto_run 的选股模板，跳过自动选股")
            return
        for t in auto_templates:
            logger.info(f"运行自动选股模板: {t.name}")
            try:
                result = service.run_screen(template_id=t.id, notify=notify)
                logger.info(
                    f"选股完成: {t.name} - 扫描 {result.get('total_scanned')} 只, "
                ...
                logger.error(f"选股模板 {t.name} 执行失败: {e}")
...
def run_full_analysis(
    ...
    try:
        ...
        # === Auto screener ===
        try:
            if getattr(config, 'screener_enabled', False):
                run_auto_screener(config)
        except Exception as e:
            logger.warning(f"自动选股失败（已忽略）: {e}")
...
def main() -> int:
    ...
    try:
        ...
        if args.schedule or config.schedule_enabled:
            ...
            # 选股器独立定时（如果配置了独立时间）
            screener_time = getattr(config, 'screener_schedule_time', '').strip()
            if screener_time and getattr(config, 'screener_enabled', False):
                def screener_task():
                    try:
                        run_auto_screener(config)
                    except Exception as e:
                        logger.error(f"定时选股执行失败: {e}")

                scheduler.schedule.every().day.at(screener_time).do(screener_task)
                logger.info(f"已设置选股器独立定时任务: {screener_time}")
