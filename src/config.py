...
class Config:
    ...
    # === 选股器配置 ===
    screener_enabled: bool = False           # 是否在每日分析后自动运行选股
    screener_schedule_time: str = ""         # 选股器独立定时时间（HH:MM），为空则跟随主任务
    screener_notify: bool = True             # 选股结果是否推送通知
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            screener_enabled=os.getenv('SCREENER_ENABLED', 'false').lower() == 'true',
            screener_schedule_time=os.getenv('SCREENER_SCHEDULE_TIME', ''),
            screener_notify=os.getenv('SCREENER_NOTIFY', 'true').lower() == 'true',
